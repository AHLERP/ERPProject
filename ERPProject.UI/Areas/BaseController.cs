...
namespace ERPProject.UI.Areas
{
    public class BaseController : Controller
    {
        ...
        protected async Task<ApiResult<List<T>>> GetAllAsync<T>(string url) where T : class
        {
            var client = _httpClientFactory.CreateClient();
            //client.DefaultRequestHeaders.Add("Header", "Bearer" + HttpContext.Session.GetString("Token"));
            ...
            if (responserMessage.IsSuccessStatusCode)
            {
                ...
                var value = JsonConvert.DeserializeObject<ApiResult<List<T>>>(jsonData);
        ...
        protected async Task<ApiResult<T>> GetAsync<T>(string url) where T : class
        {
            ...
            {
                var jsonData = await responserMessage.Content.ReadAsStringAsync();
                var value = JsonConvert.DeserializeObject<ApiResult<T>>(jsonData);
