...
namespace ERPProject.UI.Areas
{
    public class BaseController : Controller
    {
        ...
        protected async Task<bool> DeleteAsync(string url, long id)
        {
            var client = _httpClientFactory.CreateClient();
            client.DefaultRequestHeaders.Add("Header", "Bearer" + HttpContext.Session.GetString("Token"));
            var responserMessage = await client.DeleteAsync($"https://localhost:7006/api/Category/{id}");
        ...
        protected async Task<Sonuc<T>> GetAsync<T>(string url) where T : class
        {
            var client = _httpClientFactory.CreateClient();
            client.DefaultRequestHeaders.Add("Header", "Bearer" + HttpContext.Session.GetString("Token"));
            var responserMessage = await client.GetAsync(url);
            ...
            {
                var jsonData = await responserMessage.Content.ReadAsStringAsync();
                var value = JsonConvert.DeserializeObject<Sonuc<T>>(jsonData);
